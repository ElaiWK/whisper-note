...
const kVisited='zgVisitedV2', kCustom='zgCustomV1', kOrder='zgOrderV1';
...
function dayPlaces(day){let arr=[...day.places,...(custom[day.id]||[])];let order=orders[day.id];if(order){arr.sort((a,b)=>(order.indexOf(a.id)<0?999:order.indexOf(a.id))-(order.indexOf(b.id)<0?999:order.indexOf(b.id)))}return arr}
function map(q){return 'https://www.google.com/maps/search/?api=1&query='+encodeURIComponent(q)}
function save(){localStorage.setItem(kVisited,JSON.stringify(visited));localStorage.setItem(kCustom,JSON.stringify(custom));localStorage.setItem(kOrder,JSON.stringify(orders))}

function render(){
 const root=document.getElementById('cards');
 root.innerHTML=baseDays.map(day=>{
  const places=dayPlaces(day).map(place=>`<li class="place ${visited[place.id]?'checked':''}" draggable="true" data-place="${place.id}" data-dayid="${day.id}"><div class="place-line"><button class="checkbtn" data-check="${place.id}" type="button">${visited[place.id]?'✓':''}</button><button class="place-toggle" data-toggle="${place.id}" type="button"><span class="pin">${place.icon}</span><span>${place.name}</span></button><a class="mapbtn" href="${map(place.query||place.name)}" target="_blank" rel="noreferrer">Mapa</a></div><div class="place-info"><p><b>História:</b> ${place.info||'Paragem adicionada por vocês. O passado ainda não entregou relatório.'}</p><p><b>Curiosidade:</b> ${place.curiosity||'Paragem personalizada, logo oficialmente mais importante do que metade dos monumentos.'}</p><p class="distance"><b>Distância:</b> ${place.custom?'Ordem manual. Abre no Maps para recalcular sem inventar ciência.':place.next}</p></div></li>`).join('');
...
function updateProgress(){const total=baseDays.reduce((s,d)=>s+dayPlaces(d).length,0);const done=Object.values(visited).filter(Boolean).length;document.getElementById('progressText').textContent=`${done} / ${total} locais visitados`}
function bind(){
